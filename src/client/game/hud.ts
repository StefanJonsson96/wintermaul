...
import { type EndStats } from '../../shared/protocol';
...
import { clear, fmt, h, hideTooltip, tooltip } from '../ui/dom';
...
import { openEndDialog, openGiftDialog, openRacePicker, openSettingsDialog, openSetupDialog } from './dialogs';
...
export class Hud {
  ...
  private setup: { modal: ModalHandle; refresh: () => void } | null = null;
  ...
  openRacePicker(): void {
    ...
    this.raceModal = openRacePicker(this.view, () => (this.raceModal = null));
  }
  ...
  openSetup(): void {
    if (this.setup) return this.setup.refresh();
    this.setup = openSetupDialog(this.view, () => (this.setup = null));
  }

  closeSetup(): void {
    this.setup?.modal.close();
  }

  openGift(to: number): void {
    openGiftDialog(this.view, to);
  }

  openSettings(): void {
    openSettingsDialog(this.view);
  }
  ...
  showEnd(victory: boolean, stats: EndStats, isHost: boolean, actions?: EndAction[], extra?: HTMLElement): void {
    ...
    this.endModal = openEndDialog(this.view, victory, stats, isHost, actions, extra);
  }
